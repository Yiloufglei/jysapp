<template>
    <div class="symbol-item flex_ul flex col-center" @click="toChart(source.symbol)">
        <div class="list_l">
            ...
            <p class="list_text_b">24H额：{{$numberFormat(((source.info.amount * source.info.quote_price * (appConfig.option || 1)) || 0))}}</p>
        </div>
        <div class="list_c">
...
export default {
    ...
    methods: {
        toChart (symbol) {
            this.$store.commit('SET_HQSYMBOL', symbol.split("/").join("_").toLowerCase())
            this.$router.push({
                path: "/binaryOptionTrade"
            });
        },
